...
export interface OrderCompletionData {
  order_id: number
  fulfilled_on: string
  order_batch_data: OrderBatchData[]
}
...
export function OrdersTable({ orders, loading, onComplete, completingOrderId, orderBatchData }: OrdersTableProps) {
  ...
  const handleOrderComplete = (success: boolean) => {
    if (success && selectedOrderId && onComplete) {
      onComplete(selectedOrderId)
    }
  }
